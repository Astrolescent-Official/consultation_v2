  ...
  'VotePowerSnapshot',
  {
    ...
    effect: Effect.gen(function* () {
      ...
      return Effect.fn('VotePowerSnapshot')(function* (input: {
        ...
        const needsLsulpRate =
          sourceConfig.sources.has('lsulp') || hasPoolPositions
        ...
        return yield* Effect.gen(function* () {
          ...
          const lsuConverterMap = yield* needsLsuMap
            ? buildLsuConverterMap(stateEntityDetails, input.stateVersion).pipe(
                Effect.tap((map) =>
          ...
          const { votePower, breakdown } = pipe(
            ...
            A.reduce(
              ...
              (acc, address) => {
                ...
                const xrd = sourceConfig.sources.has('xrd')
                  ? computeXrd(address)
                  : zero
                const lsu = sourceConfig.sources.has('lsu')
                  ? computeLsu(address)
                  : zero
                const lsulp = sourceConfig.sources.has('lsulp')
                  ? computeLsulp(address)
                  : zero
                ...
                const accountBreakdown: PoolContribution[] = [
                  ...
                  ...R.get(poolUnitResult.breakdown, address).pipe(
                    Option.getOrElse((): readonly PoolContribution[] => [])
                  ),
