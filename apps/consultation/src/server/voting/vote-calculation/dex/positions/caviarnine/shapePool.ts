...
import { type BinMapData, GetQuantaSwapBinMap } from './binMap'
  ...
  'CaviarNineShapePosition',
  {
    ...
    effect: Effect.gen(function* () {
      ...
      return Effect.fn('CaviarNineShapePosition')(function* (input: {
        ...
        const componentStates = yield* getComponentState
          .run({
          ...
          .pipe(
            Effect.catchTag('EntityNotFoundError', (error) =>
              Effect.gen(function* () {
                ...
                return [] as {
                  address: string
                  state: s.infer<typeof QuantaSwapSchema>
                }[]
              })
            )
          )
          ...
          (cs) =>
            Effect.gen(function* () {
              ...
              const binMap = yield* getQuantaSwapBinMap({
                ...
                Effect.catchTag(
                  'EntityNotFoundError',
                  (): Effect.Effect<BinMapData> => Effect.succeed(new Map())
                )
              )
        ...
        return yield* Effect.reduce(
          input.nftBalances.items,
          {
            ...
            breakdown: R.empty<AccountAddress, readonly PoolContribution[]>()
          },
          (acc, accountNfts) =>
            Effect.gen(function* () {
